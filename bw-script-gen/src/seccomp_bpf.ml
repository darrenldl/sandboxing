...
|};
      ...
      List.iter (fun x ->
          ...
          write_line
            (Printf.sprintf "  if (seccomp_rule_add(ctx, SCMP_ACT_KILL, SCMP_SYS(%s), 0) < 0) { goto out; }" s);
        ) blacklist;
      write_line "";
      write_line (Printf.sprintf "  filter_fd = open(\"%s_seccomp_filter.bpf\", O_WRONLY)" name);
