...
gcc "$script_dir"/../seccomp-bpfs/deluge.c -lseccomp -o "$script_dir"/../seccomp-bpfs/deluge.exe
"$script_dir"/../seccomp-bpfs/deluge.exe
if [[ $? != 0 ]]; then
  echo "Failed to generate seccomp filter"
  exit 1
fi

mv deluge_seccomp_filter.bpf "$script_dir"/../seccomp-bpfs
...
( exec bwrap \
  ...
  --unshare-cgroup \
  --new-session \
  --seccomp 10 10<"$script_dir"/../seccomp-bpfs/deluge_seccomp_filter.bpf \
